    ...
    <a-form :form="form" layout="inline" style="voerflow:hidden;" @keyup.enter.native="update">
      ...
      <a-col :span="12">
        <a-form-item label="毛坯数量">
          <a-input v-decorator="['embryoNum']" disabled style="width:320px;"></a-input>
        </a-form-item>
      </a-col>
      <a-col :span="12">
        <a-form-item label="每毛坯件数">
          <a-input
            ...
            v-decorator="['inTheEmbryoComponents',{rules: [{ required: true, message: '请填写每毛坯件数' },{validator: chickNumber1}]}]"
            style="width:320px;"
            @change="geteMbryoNum"
          ></a-input>
        </a-form-item>
        ...
        <a-col :span="12">
          <a-form-item label="毛坯外形尺寸长">
            <a-input
              ...
              placeholder="单位:mm"
              oninput="if(value.length>10)value=value.slice(0,10)"
              v-decorator="['length1',{rules: [{ required: true, message: '请填写长' },{validator: chickNumber}]}]"
              style="width:151px;"
            ></a-input>&nbsp;±
            <a-input
              type="number"
          ...
          <template slot="workTypeId" slot-scope="text, record, index">
            <div>
              ...
              >
                <a-select-option
                  :value="i.id"
                  v-for="i in processTypesList"
                  :key="i.id"
                >{{i.workTypeName}}</a-select-option>
              </a-select>
            </div>
          </template>
  ...
  methods: {
    ...
    chickNumber(rule, value, callback) {
      if (
        ...
        callback(new Error("请输入大于等于0的数值，且只能保留两位小数"));
      } else {
        callback();
      }
    },
